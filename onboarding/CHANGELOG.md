...
- ⏳ Далі: sync `bot_subscribers` з SendPulse (READ) + композер у ретеншн-редакторі (конструктор кнопок/відеозамітка/медіагрупа/форматування/тумблер DM-only) + токен ПУБЛІЧНОГО бота для прод-розсилки.
