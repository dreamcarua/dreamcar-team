...
## 10.08.2026 — Фінанси: вкладка «Додаткові надходження» (запит Вадима)

### dashboard.dreamcar.ua/finance
- 10.08.2026 🚗 Автосвіт: аудит 3 агентами + раунд фіксів — TZ publish_at, claim-гейт апрува, RLS закрито, spotlighting/ретраї/дедуп у конвеєрі, метрики-луп 72h (autosvit-daily-ops, cron 7120), TG-апрув кнопками через SMM-бота (гілка av:), Threads-версії, A/B гачки, слайди каруселей, пакет для зйомки на /autosvit/
- 🆕 Вкладка **💰 Додаткові надходження** (крипта / ФОП / інше) — ручні рядки: дата, тип, сума (гросс), джерело, нотатка. Таблиця з чистими сумами + тотали (гросс / відрахування / чисте).
- 🧮 Правила розрахунку (узгоджено): **крипта** — net = сума × (1 − власний % запису); **ФОП** — net = сума × (1 − Σ активних `percent_rates`, зараз 12%) «по звичайній схемі»; **інше** — повна сума.
- 📊 **Фолд у P&L Огляду**: консистентне подвійне проведення — revenue += гросс, cost += відрахування, net += чисте; під Net Profit — позначка «у т.ч. дод. надходження: +X ₴». Ядрові функції P&L не змінювались (окремий шар).
- 📖 Міграція `docs/finance/db/021_additional_income.sql`: таблиця `additional_income` + `dashboard_additional_income(from,to)` + `fin_system_rate_at(date)`; RLS read=team, write=cfo+.
- ✅ Chrome smoke на проді: sysRate=12% з БД, крипта 1000×10%→900 ✓, Огляд foldив +900, тест прибрано. 0 console-помилок.

## 10.08.2026 — SMM: «Чек-лист» + «Базовий контент-план» (запит Олександра)

### HQ · нова секція меню «Шаблони проєкту»
- 🆕 **Сторінка «✅ Чек-лист»** (`#checklist`, `hq/app-checklist.js`). Дві зони:
  - *Базовий шаблон* — незмінний майстер-список по секціях (додати/редагувати/вимкнути/видалити/**drag&drop**). Пункт можна «вимкнути» — тоді він не потрапляє в нові чек-листи.
  - *Чек-листи проєктів* — кнопка «Створити чек-лист для проєкту» (RPC `checklist_create_from_template`) копіює активні пункти шаблону в інстанс, привʼязаний до циклу (`launches`). В інстансі — чекбокси з **autosave**, **progress bar**, лічильник «X з Y», «Скинути галочки», виконані візуально гасяться + закреслюються + «✓ Виконано». Оригінальний шаблон лишається незмінним.
- 🆕 **Сторінка «🗂 Базовий контент-план»** (`#content-plan`, `hq/app-content-plan.js`) — бібліотека публікацій **без дат**, згрупована за етапом (`stage`) + порядковий №. Картка: назва, опис, тип, категорія, рубрика (колір), **посилання-референс**. CRUD + drag&drop.
  - «＋ В календар» копіює картку у `publications` як **чернетку** на старт обраного проєкту з міткою `work_status='Потребує дати'` (через рідний `Store.upsertPub` — зʼявляється одразу). Картка в шаблоні лишається.
  - Масовий вибір чекбоксами → «Додати вибрані в проект» (bulk). Селектор «Куди додавати» = цільовий цикл; дати призначаються потім у Календарі.
- 📖 Міграція `hq/db/migrations/020_checklist_and_content_plan.sql`: таблиці `checklist_template_items`, `checklist_projects`, `checklist_project_items`, `base_content_items` + RPC + RLS (патерн `current_user_id`/`current_user_has_role`, як 019). Засіяно стартовий шаблон (14 пунктів) і скелет контент-плану (7 карток) — редаговані.
- ✅ Chrome smoke: обидві сторінки рендеряться на проді, 0 console-помилок; RPC create-from-template + cascade перевірені.

## 10.08.2026 — Горизонт 0: розблокування (після ревізії року)

> Ревізія року: [BACKLOG_2026-08.md](BACKLOG_2026-08.md). Головний висновок — усе, що в трекері, виконано; усе, що лишалось у доках, вмирало (Хвиля 1: 2 з 20).

### Ретеншн · сегментація і захист бази
- 🛡 **`sendpulse-subscribers-sync`: відписані більше не «активні».** Було `is_active = !banned` → 1 138 відписаних (`unsubscribed_at`/`status=4`) лишались активними і полетіли б у розсилку (спам, ризик бану бота). Тепер `is_active = !banned && !unsubscribed_at`; backfill наявних 14 516 → чиста база для розсилки **13 378**.
- 🆕 **`user_status` сегмент** із survey-змінної `trust`: `loyal` (1 239) / `new` (107) / `cold` (15). Воркер `retention-bot-broadcast` фільтрує по ньому. `tariff` поки порожній (нема в SendPulse — потрібен join з deals, H2). Чек-ліст: [RETENTION_BROADCAST_LAUNCH.md](RETENTION_BROADCAST_LAUNCH.md).

### Тех-звернення · серверний нагляд
- 🆕 **`tech-requests-watchdog`** (Edge + cron 7047, 3×/день) — DM Вадиму, якщо будь-яке звернення зависло (new/in_progress/confirmed >2год, need_info >24год, proposed >48год). Фікс кореневої причини, чому #12 висіло 4 дні (Mac був закритий). Працює 24/7 без Mac.
- 🔧 Планувальник `dreamcar-tech-requests` відновлено в реєстрі (випав 04.08).

### Meta автопост · токени
- 🆕 **`meta-token-refresh.yml`** (cron 1-го числа міс) — авто-подовження 60-денних Meta-токенів (user/threads/page) із записом назад у GH secrets + Edge env. Self-skip доки Meta-секрети не задані. ⚠️ Весь Meta-автопост наразі DARK — секрети `META_*` не встановлені.

### Цикли
- 📖 [CYCLE_LAUNCH_RUNBOOK.md](CYCLE_LAUNCH_RUNBOOK.md) — повторюваний чек-лист старту циклу (research-константи: вікно 0-5 днів, Д-7 дожим, беззбитковість pixel 3.7). №20 (X6M) досі `status='active'` попри фінал 03.08; №21 не заведено — очікує рішень Вадима.
...
- 🆕 **#3 (Олександр): сторінка «📝 Нотатки» у лівому меню HQ** (`#notes`) — ідеї для реалізації стовпчиком, поле додавання зверху (Ctrl+Enter), фільтри **На розгляді / Схвалені / Відхилені / Усі**, лічильник «на розгляді» на пункті меню.
- 🆕 Поруч з кожним рядком — рішення трьох: **Вадим (ceo) · Артем (cfo) · Давид (coo)**, кнопки ✅ Апрув / ❌ Відхилено. Кожен бачить усі голоси, але клікає лише свій; повторний клік по тій самій кнопці знімає голос.
- 🆕 Статус рахує БД (`hq_notes_recompute_status` + тригер): хоч один «Відхилено» → **rejected**; усі три «Апрув» → **approved**; інакше **pending**. Колірні стани рядка: жовтий / зелений / червоний з закресленням.
- 🛡 RLS: читають усі авторизовані, додає ідею будь-хто (тільки за себе), **голосувати може лише ceo/cfo/coo і лише за себе**. Право голосу прив'язане до ролі, не до UUID — склад можна міняти без міграції.
- 🆕 Edge **`hq-notes-notify`** + DB-тригери через `pg_net`: нова ідея → DM трьом голосувальникам; голос → DM автору ідеї з поточним рахунком (⏳/✅/❌ по кожному) і фінальним статусом. Текст HTML-escape'иться (граблі `<`/`>` у TG parse_mode=HTML).
- 📖 Міграція: `hq/db/migrations/019_hq_notes_with_votes.sql`. Фронт: `hq/app-notes.js` + route `notes` у `hq/app-core.js`.

### HQ · SMM-стіл
- 🔧 **#2 (Олександр): картка публікації закривалась при виділенні тексту мишкою.** Причина — при виділенні курсор виїжджав за межі модалки, `mouseup` стався на backdrop, і браузер слав `click` з `target === backdrop` (спільний предок down/up). Фікс: `Modal.isBackdropClick()` в `hq/app-core.js` — закриваємо лише коли **і** `mousedown`, **і** `mouseup` були на самому backdrop; `hq/app-views.js` (`openCard`) використовує ту саму перевірку.

### Dashboard · фільтри
- 🔧 **#6 (Вадим/Олександр): фільтр «Нові (перша покупка)» нічого не фільтрував.** Знайдено: RPC `dashboard_agg_deals_with_traffic` **приймав** `p_customer_type` / `p_tariff` / `p_pay_provider`, але **не застосовував** їх — MV `mv_dashboard_utm_agg` не має цих вимірів. Через це фільтр був мертвий у розділах Джерела / Тип трафіка / Кампанії / Оголошення / Виконавець.
- ⚡ Фікс без втрати швидкості: **дві взаємовиключні гілки** в одній функції — без цих фільтрів працює старий MV-шлях (50 мс), з фільтрами — точний підрахунок по `dashboard_deals` (900 мс на всю історію). Перемикання через One-Time Filter на параметрах.
- ✅ Звірка: 30 днів, `customer_type='new'` → agg **1580** = `dashboard_kpi_summary` **1580** (було: фільтр ігнорувався).
- 🔧 `dashboard_agg_by_person` (People Merge) — додано `p_customer_type` / `p_tariff` / `p_pay_provider` / `p_traffic_type` (усі `DEFAULT NULL`). Витрати реклами не показуються при розрізі по типу клієнта — spend не ділиться на new/returning, ROI був би брехливий.
- 🔧 Розділ **Проекти** більше не показує панель фільтрів: там лайфтайм по кожному проєкту за його власний `date_start..date_end`, глобальний період/тип клієнта там принципово не застосовні. Раніше панель висіла і виглядала зламаною. Підзаголовок це тепер явно пояснює.

---

## 29.07.2026 — Ретеншн: нативна DM-розсилка в бота (запит Віри)

### Retention · broadcast (Phase 1 — фундамент + воркер, тест на командному боті)
- 🔎 Знахідка: старий `retention-scheduler` (канал tg) шле **один текстовий sendMessage** на chat_id і **ігнорує** медіа/кнопки/фільтри аудиторії, які збирає UI. Тому нативну розсилку будуємо заново.
...
## 01.08.2026 — 🔴 SMM: автопост і воркфлоу погодження (скарги Олександра)

### 🔴 Автопост TG не працював ТИЖДЕНЬ (#4)
- 🚨 Симптом: заплановані пости не виходять; команда публікує руками (у БД `status='published'` без `tg_message_id`). Останній автопост — 24.07.
- 🔴 Причина — **моя регресія від 28.07**: вікно вибірки звузилось до `[now-15хв .. now]`. Публікацію зазвичай **схвалюють ПІСЛЯ** `publish_at` — і тоді вона вже поза вікном, тож не виходить **ніколи**. Cron працював справно (кожні 2 хв) — тому проблему не було видно в логах.
- 🔧 Вікно → **12 годин назад**: пост виходить одразу після схвалення, навіть якщо час минув. Старші за 12 год → `autopost_status='missed'` (не постимо мовчки застаріле). + watchdog: `processing` >20 хв звільняється для retry.
- 🔧 Після успішного автопосту статус тепер **`published` + `published_at`** — раніше лишався `approved`, тому SMM думав що пост не вийшов.
- ✅ Живий тест: тестовий пост опубліковано автоматично (msg 9911 у SMM-чат).

### 🔴 Воркфлоу погодження (#2, #3)
- 🔴 «Тисну «на погодження» — у чат не прилітає»: **часовий дедуп 5 хв** у `publications_notify_with_dedup` відсікав нотифікацію, якщо доопрацювання зайняло менше 5 хв після rework. Дублі того самого статусу й так відсікаються окремою перевіркою → часовий поріг знижено до **20 сек** (лише від подвійного кліку).
- 🔴 «Статус не переходить у погоджений, висять два статуси»: при поверненні на доопрацювання скидався лише масив `approved_by`, а записи `publication_approvers.is_approved` лишались `true`. Погоджувач вважався таким, що вже схвалив → повторний Approve **нічого не робив**, у нього UI показував «погоджено», у виконавця — «review».
- 🆕 Тригер `reset_approver_rows_on_rework` — скидає `is_approved`/`decided_at` при переході у rework/draft/in_work. Умову `reset_approvals_on_rework` розширено (було лише з `review`). Залиплі апруви на існуючих публікаціях очищено.

### AI-копірайт (#1)
- 🔧 Захист від «перекидає на новий чернетку»: hash картки запам'ятовується перед вставкою і відновлюється, якщо щось його збило (3 перевірки, остання — після debounce-autosave). Прибрано подвійний binding кнопки (onclick + addEventListener + делегат = 3 обробники на клік).
- ⏳ Першопричину не відтворено (потрібен сеанс із Сашею) — фікс захисний, симптом усуває.

### 🔴 Dashboard · BMW X6M був відсутній у реєстрі проектів
- 🚨 Симптом (Вадим): на `#projects` немає актуального X6M, а Mustang висить `ACTIVE` через місяць після фіналу.
- 🔴 Причина: реєстр `dashboard_projects` заповнюється **вручну**, а `status` — просте текстове поле без автооновлення. X6M стартував 08.07, але його не завели → **15 347 угод і 6.25 млн ₴ місяць не потрапляли на дашборд**. Mustang завершився 28.06, статус ніхто не змінив.
- ✅ Додано **BMW X6M** (08.07–02.08, active) + знайдено ще один загублений — **IPHONE 17 PRO MAX 2** (1060 угод, 02–05.07). Mustang → completed. `mv_dashboard_projects_stats` оновлено, перевірено на проді.
- 🆕 **Автостатус за датами**: функція `sync_dashboard_project_status()` + pg_cron `dashboard-projects-status-sync` (щодня 03:10). Проект у вікні дат → `active`, після `date_end` → `completed`. Ручні стани `measure`/`archived` не чіпаються.
- 🆕 **Автодетект немапнутих проектів**: RPC `find_unmapped_projects()` + Edge `detect-unmapped-projects` + cron `detect-unmapped-projects-daily` (09:20 Київ). Якщо в угодах з'явиться значення `project`, якого немає в реєстрі (≥50 угод за 30 днів) — DM Вадиму з к-стю угод і виручкою. Anti-spam: один алерт на значення. Перевірено симуляцією: до фіксу видав би «BMW X6M — 15 653 угоди, 6 401 058 ₴». (Vadym 31.07)

### Retention · кольорові кнопки + порядок блоків (правки Віри)
- 🆕 **Вибір кольору кнопки** — Telegram Bot API 9.4 (лютий 2026) додав поле `style`: 🔵 синя (`primary`), 🟢 зелена (`success`), 🔴 червона (`danger`), ⚪ звичайна. Селект у конструкторі кнопок; передається і в ретеншн-розсилках, і в SMM-постах (`tg-post-send`).
- 🆕 **Кнопки видно у прев'ю** — з реальними кольорами, оновлюються на льоту.
- 🔧 Блок **📨 TG-ОПЦІЇ перенесено** — тепер між редактором тексту і прев'ю (було внизу форми). Логічний порядок: пишеш → налаштовуєш → бачиш результат. (Vira 31.07)

### Tasks · BLOCKED (запити Давида)
- 🔴 Корінь проблеми «не можу знайти задачі статусу blocked»: на дошці було **лише 4 колонки** (inbox/doing/review/done) — задачі у `blocked` просто **зникали з очей**. Додано колонку **🚧 BLOCKED** (5 колонок) + чіп-фільтр «🚧 Заблоковані».
- 🔧 **Вимкнено нагадування для blocked** (`team-tasks-cron`): ні `reminder_24h`, ні «прострочено» більше не шлються — задача стоїть не з вини виконавця, це був спам. Щоденний дайджест лишився (це список, не пінг). (David 30.07)

### SMM · ранковий план тепер повний
- 🆕 `daily-smm-schedule` о 08:00 тепер шле **весь план соцмереж**: спершу 📣 SMM-публікації, далі 🤖 **ретеншн-розсилки в бота** (час · іконка медіа/відеозамітки · назва · готовність) з підсумком по кожному блоку.
- 🔧 Дрібні фікси формату: у назвах розсилок прибираються HTML-теги (було видно сирі `&lt;b&gt;`), день тижня у називному відмінку («пʼятниця», не «пʼятницю»). (David 30.07)

### 🔴 Каса · Спірін і Заяць вичерпали ліміт → архівні ФОП
- 🚨 Виявлено при аудиті: **Спірін 100%** (перевищення на 29 ₴ від 7 211 598), **Заяць 99.9%** (лишалось 10 018 ₴). Оборот тепер **лише через Недвигу** (12%, запас 6.35 млн).
- 🔴 Прогалина в алертах: пороги закінчувались на **99** — тобто про сам факт перевищення ніхто б не дізнався. Додано поріг **100** + заголовок «🚨 ПЕРЕВИЩЕНО ФОП-ЛІМІТ».
- 🆕 Таблиця `kasa_fop_registry` (name/is_active/archived_at/note): Спірін і Заяць → **архівні**, Недвига → активний. Алерт `kasa-fop-threshold-alert` тепер шле **лише по активних** — архівні не спамлять. RPC рахує всіх (потрібно для історії/звітності). (Vadym 30.07)

### TG · завжди ОДНЕ повідомлення (запит Віри)
- 🔧 **Текст іде описом до медіа й ніколи не ділиться на дві частини** (`retention-bot-broadcast`). Прибрано логіку follow-up-повідомлення. Якщо текст довший за підпис (1024) і додано **одне фото** — шлемо текст (до 4096) з великим прев'ю картинки зверху: візуально те саме, один меседж, текст не втрачається. Підпис у медіагрупі — завжди на першому елементі. Кнопки окремо лише в альбомі (обмеження Telegram API, не наш вибір).
- 🔧 Той самий `trimHtml` додано у **SMM-воркер `tg-post-send`** (бот і група): раніше `caption.slice(0,1024)` рвав HTML посеред тега → Telegram відхиляв пост.
- 🔧 Лічильник у композері: «⚠️ N/1024 (підпис до медіа)» з поясненням що робити. (Vira 30.07)

### Креативи · ліміт фото 2000px
- 🔧 Максимальна сторона зображення знижена **2560 → 2000 px** в обох шляхах стиснення: клієнтський (`app-client-compress.js`, `PHOTO_MAX_SIDE`) і серверний воркер (`compress-creative-worker.sh`, ImageMagick `-resize 2000x2000>`). Якість q90 без змін. Менша вага → швидша розсилка й пости. Документацію `compress.html` оновлено. (Vadym 30.07)

### Retention · редактор форматування (запит Віри «болд та quote»)
- 🆕 **Візуальна панель форматування** над текстом розсилки: **B/I/U/S**, 🫥 **спойлер** (`tg-spoiler`), ❝ **цитата**, ⌄ **розгортувана цитата** (`blockquote expandable`), `</>` код, 🔗 лінк, швидкі емодзі, 🧹 очистити. Виділив текст → клік. Гарячі клавіші Ctrl+B/I/U/K. Теги вручну більше не потрібні.
- 🆕 **Живе прев'ю** — показує повідомлення як побачить підписник (спойлери клікабельні, цитати стилізовані) + **лічильник символів** із правильним лімітом (1024 з медіа / 4096 без), жовтий на підході, червоний при перевищенні.
- 🔴 Фікс у воркері: `caption.slice(0,1024)` **рвав HTML посеред тега** → Telegram відхиляв повідомлення («can't parse entities»). Тепер `trimHtml()` — не ріже теги й закриває незакриті. Плюс якщо текст довший за ліміт і є медіа — медіа йде без підпису, а **повний текст окремим повідомленням** (до 4096) з кнопками: нічого не губиться. (Vira 30.07)
...
### SMM · таск-менеджер
- 🔧 Прибрано **авто-задачу «🚀 Опублікувати»**, що падала в таск-менеджер, коли пост ставав approved (тригер `publication_approved_to_task` на `publications`). Раніше вона нагадувала опублікувати вручну, але тепер **автопост TG публікує сам** — задача лише дублювала й заважала. `DROP TRIGGER trg_publication_approved_to_task`. Закриття задач (`auto_close`) і задача-доопрацювання після відхилення (`to_task_on_rework`) лишились — вони коректні. (David 28.07)

### Tasks · вікно постановки
- 🔧 Вікно постановки/редагування задачі (`#taskModal`) розширено **майже на весь екран** — `max-width 1100px · 95vw · max-height 95vh` (було 680px). Форма задачі велика (опис, чек-лист, прикріплення, коментарі) — тепер менше скролу, більше видно за раз. Дрібні діалоги (налаштування/допомога) лишились компактними. (David 28.07)

### SMM · календар — бронювання слоту
- 🆕 **Чекбокс «🔒 Забронювати слот у календарі»** у картці публікації (біля дати/часу). Коли ✓ — слот видно в загальному календарі як заброньований ботом, **навіть якщо текст ще не дописаний** (порожня чернетка показується пунктирною рамкою + 🔒 + «заброньовано ботом»). Нова колонка `publications.slot_reserved bool`. Відображення у всіх режимах календаря (місяць/тиждень/день/список). Так команда бачить зайнятий час наперед, поки контент готується. (David 28.07)

## 28.07.2026 — Автопост TG: прибрано затримку 8–80 хв

### SMM · autopost
- 🔴 Симптом (Олександр): пост запланований 23:50 — вийшов 00:20. На даних лаг **8–80 хв**, найбільший рівно на «круглих» годинах (09:00→80хв, 12:00→27хв, 00:00→26хв, 15:01→8хв).
- 🔧 Причина: автопост-worker крутився на **GitHub Actions scheduled cron** (`*/5`), а GH у пікові хвилини відкладає заплановані запуски на десятки хвилин. Плюс старе вікно `publish_at ≤ now+10хв` (worker міг постити наперед).
- 🆕 Прибрав GH з критичного шляху: `autopost-tg-enqueue` тепер постить **прямо через `tg-post-send`** (Edge→Edge, паралельно до 8, retry всередині). Вікно `[now-15хв .. now]` — постимо коли час настав. pg_cron прискорено до **кожні 2 хв** (jobid 5650). Затримка тепер **до ~2–3 хв** замість 8–80.
- 🗑 `tg-autopost.yml` scheduled вимкнено (лишився `workflow_dispatch` як backup). (commits 157dc86, 1404350)

## 27.07.2026 — SMM: ранковий розклад виходів о 08:00

### SMM · daily-smm-schedule
- 🆕 Edge `daily-smm-schedule` + RPC `smm_today_schedule` (Kyiv TZ) — щоранку о **08:00 Київ** (pg_cron 5319, 05:00 UTC) шле в робочу SMM-групу (`-1003933841573`) перелік **усіх** запланованих на день публікацій: `час · іконка типу · назва · готовність` (✅ вийшло / ⏳ ще не готово), з підсумком «Усього N · ще не готово K».
- Іконки: 🎬 відео, 📷 фото, 🖼 альбом, 📝 без медіа. Параметри `?dry=1` (preview), `?chat=` (override). Протестовано dry-run — 9 виходів. (commit eaedc5b)
...
### Дашборд · #overview
- 🔧 Блок «Останні 20 угод» показував лише `rows.slice(0,20)` — решта операцій за фільтром була недоступна. Тепер віддаються **всі угоди за поточним фільтром** з пагінацією (25/50/100/200/500/усі), як і задано в селекторі.
- 🆕 **Пошук по всіх полях** (`searchAll`): SendPulse ID, WC Order ID, email, телефон, тариф, усі `utm_*` — а не лише по видимих колонках. Можна знайти конкретну транзакцію за будь-яким параметром.
- 🆕 **Вивантаження CSV** у межах фільтру **і активного пошуку** (не лише поточної сторінки), з подробицями: дати створення/оплати, обидва ID, статус, сума, валюта, проект, тариф, email, телефон, тип клієнта, платіжка, повний набір utm (вкл. `utm_term` — виконавець).
- 🔧 Клік на рядок при активному пошуку тепер відкриває правильну угоду (синхронізовано фільтр). (commit 7df76da)

## 20.07.2026 — Медіа-архів: відео не долітали в «DreamCar Media»

### SMM · dc-media-archive v4
- 🔧 Архів (фіча #411, тригер `status → published`) слав `sendVideo` **по URL**, а TG по URL тягне лише **≤20МБ**. Усі відео 45–46МБ мовчки не доходили — у чат падав лише текст. Фото (0.2МБ) долітали, тому баг був неочевидний.
- 🆕 Тепер: ≤20МБ — по URL (швидко), **>20МБ — завантаженням (multipart)**; + `width/height/duration` та постер-thumbnail (вертикальні відео не плющить). Альбоми з важкими файлами теж через multipart.
- ✅ Протестовано на реальному пості: `video:45.0MB(1080x1920):multipart` — дійшло в чат.
- ℹ️ Друга причина «порожніх» записів: у частини публікацій (напр. «мем», «BOOST +3 токени») **немає прикріплених креативів** (`n_creatives=0`) — контент постили повз систему, архіву нічого слати. Тепер такі позначаються «(без прикріплених медіа)».

## 18.07.2026 — SMM: фікс «квадратного» відео в TG

### SMM · tg-post-send v16
...
### Каса · Алерти
- 🆕 Edge `kasa-fop-threshold-alert` — при досягненні порогів **10/20/…/90/95/96/97/98/99%** ліміту 2 групи шле повідомлення у групу BOARD (той самий chat, що фінзвіти: `-1003883456849`). Працює для **всіх ФОП** (Недвига + Спірін + Заяць).
- 🛡 Anti-spam: кожен поріг × ФОП × рік шлеться раз (`kasa_fop_alert_log`). Параметри `?dry=1` (preview), `?reset=1` (перепрогнати тест).
- ⏰ pg_cron `kasa-fop-threshold-alert-2h` (jobid 4359) — кожні 2 год.
- ✅ Протестовано: перший запуск надіслав у BOARD Спірін 98% і Заяць 97% (25 порогів залоговано). Недвига поки 1.5% — з'явиться на 10%.

## 18.07.2026 — Каса: третій ФОП Недвига + RPC ліміту

### Каса · ФОП
- 🆕 Заведено ФОП **Недвига**: monobank (`kasa_bank_creds label='Недвига'`) + ПриватБанк (`label='privat_nedvyga'`). Синк сам створив рахунки, підтягнув баланси й історію.
- 🆕 Робочі рахунки активні (mono ·1560, privat ·3274), порожні картки вимкнено (·0773, ·8550, ·1048 — 0 обороту).
- 🔧 RPC `dashboard_kasa_fop_limit` тепер ловить і Недвигу (`ilike '%nedvyga%'`/`'%Недвига%'`) — третій рядок у віджеті ФОП-ліміту. Дорогою виправив одруківку в детекторі внутрішніх переказів (не зачепило Заяця/Спіріна — перевірено).
- ⚠️ **Спірін 98.6% і Заяць 97.8%** ліміту 2 групи — майже стеля (лишилось ~104к і ~159к). Недвига (1.5%, запас 7.1млн) — клапан для перерозподілу обороту.
