...
- 🔧 **Прогрес-тост «немає анімації»:** `.toast-stack` мав `z-index:1000`, а `.modal-backdrop` — `10000` → тост стиску ховався ПІД модалкою публікації. Підняв `.toast-stack` до `10001`. Додав явний текст «Читаю велике відео N МБ…» на фазі читання у памʼять (раніше застрягало на 10% без пояснення → виглядало як заморозка).
