...
### Що видалено
- 🗑 **tg-webhook** — вирізано блок перехоплення тех-звернень: `TECH_TAGS` / `BOT_MENTION` / `TECH_CHATS` / `classifyTech()` / `captureTechRequest()` + call-site у головному хендлері (−133 рядки). Решта функціоналу бота (`/start`, `/approve`, `/queue`, `/my`, AI-асистент у DM, файли, голосові, погодження) не зачеплена.
- 🗑 **pg_cron job 7047** `tech-requests-watchdog` — знято з розкладу.
- 🗑 **Edge Function** `tech-requests-watchdog` — видалено.
- 🗑 **БД:** `public.v_tech_requests_open` (view) + `public.tech_requests` (16 рядків, усі `done`). Архів даних: `onboarding/archive/tech_requests_dump_2026-08-26.json`.
- 🗑 **Cowork scheduled task** `dreamcar-tech-requests` (щогодини 9-21) — видалено.

### Наслідки для команди
`#баг` / `#фікс` / `#питання` / `#ідея` та згадки `@dreamcar_team_bot` у робочих чатах більше не потрапляють у чергу і не отримують автовідповіді. Звернення — напряму Вадиму. Сторінка HQ **Нотатки** (`#notes`) працює як раніше — це окрема система.

## 11.08.2026 — Каса: банки ПУМБ + ОТП через імпорт (запит Вадима)

### dashboard.dreamcar.ua/kasa
- 🆕 Додано банк **ОТП** у безготівку (дропдаун + бейдж + CHECK `kasa_accounts_bank_check` +otp, міграція 023). Активовано наявний рахунок «ПУМБ ФОП», створено рахунок «ОТП».
- ℹ️ **Авто-синк (як моно/Приват) для ПУМБ/ОТП неможливий:** моно/Приват мають прості токен-API; ПУМБ має Open Banking API (перший авторизований НБУ банк, 2026), але для авто-тягнення DreamCar має бути авторизованим НБУ **TPP** (регульований статус). ОТП — лише корпоративний Click OTPay з ЕЦП. → реальний шлях: **імпорт виписки CSV/XLSX** (вже в Касі, тепер згадує й ОТП). Операції підтягуються так само, лише завантаження ручне.

## 24.08.2026 — GitHub Pro: безпека + розбір вичерпаних Actions-хвилин

### Хто насправді з'їв 2000 хвилин — не DreamCar
Actions безкоштовні й **безлімітні** для public-репо на standard runners (підтверджено docs.github.com). Обидва DreamCar-репо публічні, тож попри ~3700 хв/міс сумарного часу вони коштують **0**. Квоту з'їли приватні репо інших проєктів: `abrisart-dashboards` (2004 хв) + `sneco` (1154 хв) = 3158 хв, що не влазить навіть у Pro 3000.

🔴 **Головна пастка — округлення вгору по кожному job:** ран на 15 секунд = 1 біла хвилина. Тому 500 коротких ранів `crm-notes-retry` (554 хв) коштують більше, ніж 31 важкий `kpi-warehouse-sync` (445 хв). Часті легкі крони дорожчі за рідкі важкі.

### Безпека (обидва публічні репо)
- 🛡 **Закрито секретний алерт #1** — Telegram Bot Token у публічному репо з 22.04. Перевірено: `getMe` → 401, у поточному коді відсутній, лишився лише в історії. Позначено `revoked`.
- 🛡 **Перевірено на витік ключів**: усі знайдені JWT — `role: anon` (публічні за дизайном). Жодного `service_role`, жодного бот-токена в HEAD.
- 🛡 **GITHUB_TOKEN default → `read`** у `dreamcar-team` (був `write` + право апрувити PR). Безпечно: всі три воркфлоу, що пишуть у репо, вже мають явний `contents: write`.
- 🛡 **Dependabot alerts + security updates** увімкнено на обох (були вимкнені).
- ✅ Перевірено головний ризик публічного репо — тригерів `pull_request_target` / `issue_comment` немає, тож чужий форк не може запустити воркфлоу з нашими секретами.

### Стабільність черги
- ⚡ **`timeout-minutes` додано у 23 воркфлоу**, де його не було (7 у team, 16 у dashboard). Без нього зависла job тримає слот до **6 годин** за дефолтом GitHub, а конкурентних слотів на весь акаунт лише 20 — тобто одна зависла задача гальмує крони обох репо. 15 хв дефолт, 30 хв для важких Meta-операцій.

### Спостереження
Заявлений розклад ≠ фактичний: `ETL Facebook Ads` з `*/15` реально йде **раз на ~30 хв**. Черговий доказ правила про ненадійність GH cron — критичні речі мають ходити через push/DB-тригер.

### Нічний throttle (крок до приватизації репо)
- ⚡ `smm-content-watchdog`: було `*/30 * * * *`, хоча скрипт сам мовчить 23:00–07:00 Kyiv — третина ранів стартувала лише щоб нічого не зробити. Крон приведено до робочого вікна скрипта (`*/30 4-20 UTC`).
- ⚡ `fb-ads-sync` (найдорожчий, 1257 хв/міс) і `google-ads-sync`: удень 2 рани/год, вночі (01:00–05:00 Kyiv) — 1/год. Ранковий синк однаково добирає повну добу.
- 🚫 `etl-mysql-sync` навмисно **не чіпали** — це оплати, люди купують вночі.
- Разом ~−190 хв/міс. Чесно: це 5%, бо решта часу — реальна робота ETL і ffmpeg, а не оверхед.

### Топ-10 споживачів DreamCar і головна причина — округлення

| # | воркфлоу | хв/міс | ранів | сер. с | робота | **марно** |
|---|---|---|---|---|---|---|
| 1 | ETL Facebook Ads | 1251 | 356 | 181 | 1074 | 177 |
| 2 | Compress Creative Worker | 1053 | 298 | 186 | 924 | 129 |
| 3 | ETL MySQL | 324 | 324 | 21 | 113 | **211** |
| 4 | Meta Autopost Worker | 308 | 308 | 13 | 67 | **241** |
| 5 | TG Post Stats (MTProto) | 226 | 226 | 18 | 68 | **158** |
| 6 | SMM Content Watchdog | 158 | 158 | 15 | 40 | **118** |
| 7 | Google Ads sync | 128 | 128 | 9 | 19 | **109** |
| 8 | Cowork → TG Notify | 31 | 31 | 12 | 6 | 25 |
| 9 | ETL Instagram Insights | 20 | 8 | 123 | 16 | 4 |
| 10 | Meta Stats (daily) | 15 | 4 | 201 | 13 | 2 |
| | **разом** | **3514** | | | 2340 | **1174** |

🔴 **Третина всього споживання (1174 хв) — це округлення вгору, а не робота.** Позиції 3-7 виконуються за 9-21 секунду, але кожен ран білиться як повна хвилина. Тобто ~840 хв/міс платяться буквально за ніщо.

Висновок: різати треба не «важкі» задачі, а **рій коротких**. Їхнє природне місце — Supabase Edge Functions + pg_cron (там уже працює 57 функцій), а не Actions. Виняток — `TG Post Stats`: MTProto потребує Telethon, на Deno не переноситься.

Також: `pages build and deployment` (91 ран) у квоту **не рахується взагалі** — GitHub Pages збірки безкоштовні на будь-якому плані.

- ⚡ `smm-content-watchdog` за запитом Вадима зменшено ще вдвічі — раз на годину замість кожних 30 хв (пороги алертів 3 год і 24 год, тож реакція не постраждала).

### Ревізія: три воркфлоу працювали вхолосту (−662 хв/міс)
Перевірка «чи є в цього живий споживач» показала, що частина топ-10 не робить **нічого**:

| воркфлоу | хв/міс | що показала перевірка | дія |
|---|---|---|---|
| Meta Autopost Worker | 308 | кожен ран пише `Meta secrets not configured yet — skip` — 308 разів на місяць прокидався лише щоб повідомити, що не налаштований | 🗑 disabled |
| TG Post Stats (MTProto) | 226 | у `tg_post_analytics` 16 рядків за 30 днів, **усі з `views=0` і `clicks=0`**, останній синк 15.08 | 🗑 disabled |
| Google Ads → ads_data | 128 | **0 рядків і 0 витрат за 30 днів** — кампаній у Google Ads немає | 🗑 disabled |

Вимкнено через `gh workflow disable` (не видалено — вмикається однією командою назад).

### Підсумок чистки: −892 хв/міс (−25%)
`3514 → 2622 хв/міс`: три мертві воркфлоу (−662), watchdog з 30 хв на годину + нічне вікно (−105), нічний throttle FB/Google Ads (−125).

**Наступний рубіж — ETL Facebook Ads (~1126 хв) і Compress (1053) = 83% залишку.** Виявлено, що ETL возить нулі з 18.08: після фіналу «3 iPhone» 16.08 реклама зупинена (spend 16.08 = 35 254 ₴, 18–21.08 = 0), а воркфлоу продовжує ходити 12 разів на добу. Тобто його інтенсивність має слідувати за циклом промо, а не бути постійною.

### ETL Facebook Ads: 179 с → 15 с (порядок операцій був перевернутий)

**Що воркфлоу робить.** Ходить у Facebook Marketing API по трьох рекламних акаунтах і тягне статистику **на рівні окремого оголошення з розбивкою по днях** (`level=ad`, `time_increment=1`): витрати, покази, кліки, охоплення, CTR/CPC/CPM, конверсії. Окремо для кожного оголошення дістає `url_tags` — «Параметри URL» у Facebook, звідки береться `utm_term` з іменем медіабаєра. Пише в `dashboard_ads_data`. Це фундамент рекламної аналітики: ROAS/ROI по проєктах, розділ «Виконавець» (чий результат), CAC, графіки витрат — 10 місць у дашборді.

**Що було не так.** Спершу викачувався **весь список оголошень акаунта** (пагінація по 200) заради `url_tags`, і лише потім бралися insights. Тобто коли реклама вимкнена і показів нема, скрипт усе одно щоразу тягнув сотні оголошень — три хвилини роботи заради нуля рядків. Живий приклад: після фіналу «3 iPhone» 16.08 spend впав у нуль, а ETL продовжував ходити 12 разів на добу і робити це вхолосту.

**Фікс.** Insights беруться першими; якщо рядків нема — акаунт пропускається за секунди. Під час активного промо поведінка не змінюється. Перевірено живим прогоном: `179 с → 15 с`, усі три акаунти коректно пропущені.

Очікувана економія ~595 хв/міс при типовому циклі (≈10 днів активної реклами на місяць).

- 🔧 Заодно прибрано назви, що брехали: `every 15min` і `2 iterations × 15 min = 30 min` — internal loop прибрали ще в серпневому аудиті, а підписи лишились і збивали з пантелику.

### 🔴 ETL тригериться з ДВОХ каналів — cron у .yml сам по собі нічого не вирішує
Під час зниження частоти FB Ads з'ясувалось, що ETL-воркфлоу запускаються з двох незалежних джерел:

1. `schedule:` у самому `.github/workflows/*.yml`
2. **pg_cron → Edge `etl-trigger` → GitHub `repository_dispatch`** — `cron.job` id **75** (`etl-mysql`) і **76** (`etl-fb-ads`), обидва `15,45 * * * *`

Другий канал додали свого часу через ненадійність GH cron. Наслідок: зміна `cron:` у воркфлоу дає лише половину ефекту — решта ранів приходить повз нього. ETL MySQL фактично тригериться **96 разів на добу** (48 cron + 48 dispatch), а не 48.

- ⚡ **Міграція 025**: `pg_cron` job 76 (`etl-fb-ads`) переведено `15,45` → `45 * * * *`, тобто вдвічі рідше — синхронно зі зміною в воркфлоу (43 → 22 слоти/добу). Job 75 (`etl-mysql`) **не чіпали** — це оплати, свіжість критична.
- ⚡ ETL FB Ads за запитом Вадима: удень 1 ран/год замість двох, вночі раз на 2 год. Разом із фіксом skip-коли-немає-показів це знімає ~950 хв/міс із найдорожчого воркфлоу.

### ETL MySQL: schedule став резервом, основний канал — pg_cron

**Що воркфлоу робить** (уточнено — у репо це ніде не було описано): синкає MySQL `dreamcar_utm` (хостинг `fincheck.mysql.network`) → Supabase. Це **не** магазин, а власний UTM-трекер:

| MySQL | → Supabase | зміст |
|---|---|---|
| `crm_deals` | `dashboard_deals` | угоди **SendPulse CRM** (`deal_id` → `sendpulse_deal_id`) |
| `webhook_log` | `dashboard_webhooks` | сирі вебхуки трекера |
| `ads_data` | `dashboard_ads_data` | legacy-канал реклами |

Ланцюг: SendPulse CRM → вебхуки → MySQL `dreamcar_utm` → цей ETL → Supabase → дашборд.

**Проблема.** Тригерився з обох каналів: `cron: 0,30` (48/добу) + pg_cron job 75 (48/добу) = **96 тригерів на добу замість потрібних 48**. Кожен ран відпрацьовує 21 секунду, а білиться як повна хвилина.

**Фікс.** `schedule` знижено до 3 разів на добу (`5 3,11,19`, зі зсувом щоб не збігатись з pg_cron :15/:45) — тепер це лише резерв на випадок падіння Edge. Основним лишається pg_cron: перевірено, останні 5 запусків `succeeded`, ходить рівно за розкладом.

Заодно виправлено шапку файлу — там досі описувався internal loop 12×5 хв, якого немає з серпня, і не було жодної згадки про другий канал тригерів.

**Перевірка живого рану:** `deals 0, webhooks 0, ads 0` за 8 секунд — ETL справний, просто після завершення промо 16.08 нових угод немає. Тобто 96 разів на добу він прокидався, щоб сказати «0 рядків».

### Підсумок чистки Actions за сесію: ~3514 → ~1520 хв/міс (−57%)
| −662 | три мертві воркфлоу (Meta Autopost, TG Post Stats, Google Ads) |
| −950 | ETL FB Ads: skip коли показів нема + удвічі рідше в обох каналах |
| −230 | throttle (watchdog на годину, нічні вікна) |
| −154 | ETL MySQL: прибрано дублювання каналів |

### 🔒 Усі DreamCar-репо переведено у PRIVATE

Закрито п'ять: `dreamcar-team`, `dreamcar-dashboard`, `dreamcar-finance`, `dreamcar-audience-dashboard`, `dreamcar-planning`. Останні три були мертві (коміти березень–червень, без власних доменів), але лежали відкритими — зокрема `dreamcar-audience-dashboard` із `comments.json`, де зібрані IG-нікнейми учасників опитування.

**Публічним свідомо лишився `brand-book`** — він живить `brand.dreamcar.ua/assets/global-header.js`, який підключають усі сторінки обох столів. Його закривати не можна.

**Перевірено після перемикання:**

| що | результат |
|---|---|
| team.dreamcar.ua (hq / retention / tasks) | 200 |
| dashboard.dreamcar.ua (/ kasa / finance / meta-analytics) | 200 |
| `brand.dreamcar.ua/assets/global-header.js` | 200 |
| анонімний доступ до коду (`raw.githubusercontent`, github.com) | **404** ✅ |
| Actions у приватних репо | Compress, ETL FB Ads, ETL MySQL — усі `success` |
| Pages | `built`, CNAME збережені |

Доступи вже були чисті: жодного стороннього колаборанта, deploy-ключа чи вебхука — тільки акаунт власника.

### 🔴 Уточнення цифр: я завищив споживання на 31%
Мої підрахунки через `gh run list` давали 2706 хв, реально в Billing — **2060 із 3000**. Причина: wall-clock від `startedAt` до `updatedAt` включає час очікування в черзі, а GitHub білить по кожному job окремо без черги. **Реальні цифри брати тільки з Settings → Billing → Usage.**

Актуальний стан: 2060/3000, reset через 8 днів. Без приватизації прогноз був 2747 — влізало. Після приватизації темп ~137 хв/добу, тож залишку 940 хв вистачить приблизно на 7 днів із 8, що лишились.

⚠️ **Потрібна дія:** поставити budget у Billing → Manage budgets (напр. $20/міс на Actions). Без нього при вичерпанні квоти Actions зупиняться навіть за наявної картки. Повний місяць після приватизації: ~4100 хв проти 3000 → overage ~$6.6/міс.

## 27.08.2026 — 🚨 АВАРІЯ: квота Actions вигоріла, репо повернуто у public

### Що сталося
24.08 репо перевели у private з моєю оцінкою overage **~$7/міс**. Через 3 дні квота 3000 хв вигоріла **повністю** — Actions зупинились, разом з ними ETL, компресія і автопост. Репо повернуто у public, усе відновлено за ~15 хвилин.

### Корінь — помилка заміру, а не оптимізації
Я рахував споживання через `gh run list --limit 400/1000`. **Цей ліміт віддає лише останні N ранів, а не всі.** Бачив «356 ранів ETL FB Ads за місяць» і будував на цьому розрахунки.

Реальність із Billing: **`dreamcar-dashboard` — 5854 рани і 31 093 хвилини за серпень ($186.56)**. Я занизив у **20 разів**.

Причому помилявся двічі в різні боки: спершу **завищив на 31%** (wall-clock `startedAt→updatedAt` включає чергу), потім **занизив у 20 разів** (ліміт вибірки).

### Звідки 5854 рани (замір за 22-23.08, до змін)
| тригер | воркфлоу | ранів/2 доби |
|---|---|---|
| `workflow_dispatch` (pg_cron→Edge) | ETL MySQL | 96 |
| `workflow_dispatch` (pg_cron→Edge) | ETL FB Ads | 96 |
| `schedule` | SMM Watchdog | 82 |
| `schedule` | ETL FB Ads | 81 |
| `schedule` | Google Ads | 68 |
| `schedule` | ETL MySQL | 66 |

~245 ранів на добу. Нічого «зламаного» — просто багато частих воркфлоу, кожен по 1-4 хв, і **кожен білиться окремо**. Середній білінг вийшов 5.3 хв/ран проти 1-3.6 хв wall-clock.

### Чому раніше це нічого не коштувало
Public-репо на standard runners = Actions **безлімітно безкоштовні**. У Billing це видно як `$237.33 consumed − $237.35 discounts = $0`. Гроші нараховувались завжди, просто повністю покривались знижкою. Приватизація зняла знижку — і 3000 хв згоріли за 3 дні.

### Стан після відкату
- ✅ `dreamcar-dashboard`, `dreamcar-team` → **public**, Actions знову безкоштовні
- ✅ ETL FB Ads, ETL MySQL, SMM Watchdog — **увімкнено назад** (міграція 027 повернула pg_cron 75/76 до робочої частоти). Ризик «забули ввімкнути перед промо» дорожчий за хвилини, яких тепер не рахують
- ✅ Сайти 200, рани проходять
- 🗑 `Google Ads` лишається disabled — там справді 0 даних за 30 днів
- 🔒 Приватними лишаються `dreamcar-finance`, `dreamcar-audience-dashboard`, `dreamcar-planning`, `DreamCar_hiring` — вони майже не мають Actions, тож нічого не коштують

### Що лишилось корисним із цієї сесії
Оптимізації самі по собі правильні й лишаються в силі: три мертві воркфлоу вимкнено, ETL FB Ads не тягне список оголошень коли показів нема (179с → 15с), нічний throttle, знайдено другий канал тригерів (pg_cron). Помилковою була **лише приватизація, зроблена на хибних цифрах**.

### Умова для приватності в майбутньому
Щоб закрити репо без рахунку на $190/міс, треба спершу звести 5854 ранів/міс до ~500. Реальний шлях — перенести ETL MySQL і FB Ads на Supabase Edge Functions + pg_cron (там уже працює 57 функцій). Тоді в Actions лишиться тільки compress (~300 ранів/міс), і приватизація коштуватиме одиниці доларів.

---

### 27.08 (ввечері) — реальний звіт GitHub + перевірка ризиків Supabase

**Звіт за 27.08 (503 хв за добу) розставив усе по місцях:**

| хв | репо | головне |
|---|---|---|
| **312** | `nof1-app` | `build-android.yml` — 278 хв (62% усього акаунта) |
| 47 | `dreamcar-team` | з них **30 — pages** (у квоту не входять) → реально 17 |
| 45 | `dreamcar-dashboard` | etl-mysql 24, fb-ads 13 |
| 39 | `sneco` | cf-purge 16 |
| 27 | `iron-moto-cascais` | |
| 26 | `abrisart-dashboards` | customer-360 11, kpi-warehouse 9 |

Тобто DreamCar — це ~53 хв/добу з 503, а не головна проблема.

### 🔴 Перевірка лімітів Supabase ПЕРЕД переносом (щоб не зробити гірше)
| ліміт | значення | наш стан |
|---|---|---|
| **CPU time на запит** | **2 s** (async I/O не рахується) | 🔴 критично для ETL |
| Wall clock | 400 s (Pro) | ок |
| Memory | 256 MB | ок |
| Invocations | ~2M/міс | 136 cron-джобів, ~98k/міс = **5%** |

**Висновок: важкі ETL (FB Ads, MySQL) на Edge переносити НЕ МОЖНА.** Вони парсять тисячі рядків — це чистий CPU-час, і 2 s вони переберуть. Плюс pg_net уже має історію bloat. Перенос зробив би гірше, а не краще.

На Edge пішло тільки те, що справді легке:
- 🆕 **`smm-content-watchdog`** — Edge Function замість 279 рядків Python. Це два HTTP-запити й трохи арифметики, CPU-часу майже не витрачає. Секрети не в env: `fb_access_token` та `ig_user_id` читаються з `app_secrets`. Тихі години рахуються через `Intl` з `timeZone: Europe/Kyiv` — Deno за замовчуванням UTC, без цього алерти летіли б серед ночі.
- ⏸ GH-воркфлоу watchdog **поки не вимкнено**: попрацюють добу паралельно, звіримо, потім вимкнемо. Дублю алертів не буде — обидві версії читають один `smm_watchdog_state` і поважають `REMIND_HOURS=1`.
- ⚡ **ETL MySQL: 48 → 24 рани/добу** (pg_cron 75 → `30 * * * *`). Це найдорожчий DreamCar-воркфлоу за звітом (24 хв/добу). Затримка оплат зростає максимум до 60 хв; на час промо повертаємо `15,45` або тиснемо `workflow_dispatch`.

### Рекомендації по решті (не DreamCar, не чіпав)
- `sneco` → `cf-purge.yml` 16 хв/добу: очистка кеша Cloudflare, схоже, висить на кожному push. Достатньо кількох разів на добу.
- `abrisart-dashboards` → `customer-360-sync` 11 хв + `kpi-warehouse-sync` 9 хв на добу.
- `iron-moto-cascais` 27 хв/добу — для сайту-візитівки це багато.

---

### Ревізія решти репо акаунта (24.08)
Усього 14. Після чистки **10 private / 4 public**.

Публічними лишились рівно ті, що є живими сайтами на власних доменах: `brand-book` (brand.dreamcar.ua — обов'язково, віддає `global-header.js` для обох столів), `sneco-brand-book` (brand.sneco.ua), `barpi-brand-book` (brand.barpi.ua), `iron-moto-cascais` (ironcustommotors.com).

- 🔒 `DreamCar_hiring` закрито — єдиний публічний без власного домену. Персональних даних у `DreamCar_Survey_Results.html` немає (перевірено), лише агреговані результати. Код тепер 404, Pages-сайт лишився живим (200).

**Знайдено поза DreamCar (не чіпав, рішення власника):**
- 🔴 `iron-moto-cascais` — відкритий секретний алерт **Google API Key** з 05.05.2026 у `worker/README.md`. Іронія: той самий README попереджає «якщо ключ колись комітили в цей публічний репозиторій, відкличте його в Google Cloud». Воркер переробили на Cloudflare-секрет, а ключ так і не відкликали. Живість не перевіряв — не слати чужий ключ у зовнішній API.
- ⚠️ `sneco-brand-book` — 334 МБ, крім гайдлайнів у відкритому доступі лежать `documents/{legal,export,markets,certificates}`, `dashboard/LAUNCHER_CARDS_FOR_VADYM.md` і `equipment-photos/`.

### 🔍 Аудит закритості після приватизації — що реально бачить сторонній

Закриття репо ≠ закриття продукту. Перевірено все, що доступно ззовні:

| вектор | стан | деталі |
|---|---|---|
| Код бекенду (ETL, Edge, workflows, SQL) | 🟢 закрито | `raw.githubusercontent` і github.com → **404** анонімно |
| **Дані** (PostgREST з anon key) | 🟢 закрито | `dashboard_deals/projects/ads_data` → **401 permission denied**; `users`, `launches`, `kasa_transactions`, `publications`, `creatives`, `retention_messages` → 200, але **0 рядків** (RLS тримає) |
| Edge Functions без секрету | 🟢 закрито | 401 / 405 / 410 — жодна нічого не віддає |
| Архівні копії | 🟢 чисто | Wayback: **жодного знімка** ні репо, ні сайтів |
| Індексація пошуком | 🟢 закрито (24.08) | team уже мав `noindex`; дашборду **не було** — додано у 7 сторінок + `robots.txt Disallow` |
| **Фронтенд-код** | 🔴 **публічний** | `dashboard.dreamcar.ua` віддає **279 КБ HTML** будь-кому без логіну |

🔴 **Головне обмеження, яке приватизація не лікує.** GitHub Pages — статичний хостинг без авторизації: HTML і JS віддаються до будь-якої перевірки логіну. `auth-guard.js` ховає *дані*, але не *код*. Тому сторонній, який просто відкриє view-source, бачить: назви всіх таблиць і RPC (`dashboard_deals`, `dashboard_agg_by_person`, `dashboard_cohort_retention`…), структуру фільтрів, логіку розрахунків ROAS і атрибуції по `utm_term`, ref Supabase-проєкту.

Чого **не** бачить: самих цифр (виручка, витрати, учасники), ETL-логіки, Meta-стратегії, скриптів масштабування, секретів — усе це тепер у приватних репо або за RLS.

**Що закрило б і це:** Cloudflare Access перед доменами (CF уже використовується — `CF_API_TOKEN`/`CF_ZONE_ID` є в секретах). Тоді без логіну не віддається навіть HTML. Альтернатива — перенести столи з Pages на Cloudflare Pages з Access.

### Арифметика приватизації репо (станом на 24.08)
| | хв/міс |
|---|---|
| DreamCar (обидва репо, після throttle) | 3523 |
| abrisart-dashboards + sneco | 3158 |
| Квота Pro | 3000 |

🔴 **Приватні репо інших проєктів вже перевищують квоту Pro на 158 хв — без жодної приватизації DreamCar.** Тому закрити DreamCar-репо в межах квоти неможливо в принципі: потрібна або картка (~$22/міс за overage), або self-hosted runner на Mac для двох найважчих воркфлоу (`fb-ads-sync` + `compress`, разом 2312 хв → 0, тоді overage ~$8/міс), або скорочення abrisart/sneco.

---

## 17.08.2026 — Виручка «Три iPhone 17» була завищена вдвічі (alias-колізія)

### Симптом
У «Усі проекти — деталі» проєкт показував **6747 оплат / 1 410 905 ₴** при реальних **2874 / 685 589 ₴**.

### Причина
У `dashboard_projects.deal_project_values` проєкту `3iphone` було значення `IPHONE 17 PRO MAX` — але воно вже належало червневому промо `iphone_17_jun2026`. MV `mv_dashboard_projects_stats` джойнить угоди **тільки** по `d.project = ANY(vals)`, **без жодного фільтра по датах**. Тож новий проєкт забрав собі всі оплати старого, а ті самі 3873 угоди (725 316 ₴) рахувались **одночасно у двох проєктах** — отже й KPI «Виручка lifetime» був завищений на цю суму. ROI/CAC/AOV брехали слідом.

Значення вписуються руками у формі проєкту, і назва «IPHONE 17 PRO MAX» природно проситься для промо, де один із трьох призів саме такий. Це граблі, на які наступлять знову — тому не лише фікс даних.

### Зроблено (міграція `024_project_alias_collision_guard.sql`)
- 🔧 Прибрано чуже значення з `3iphone`. Перевірено: угоди з `project='IPHONE 17 PRO MAX'` існують лише 05-07.06 і 02-05.07, у серпні — жодної.
- 🛡 Тригер **`trg_project_alias_collision`**: одне alias-значення = рівно один проєкт. Спроба перетину падає з `unique_violation` і текстом, який пояснює наслідки. Перевірено тестом — спрацьовує.
- ⚡ `REFRESH MATERIALIZED VIEW CONCURRENTLY` (MV легка, 64 kB; крон jobid 55 і так рефрешить двічі на годину).

### Після фіксу
`Три iPhone 17` → 2874 оплати · 685 589 ₴ · AOV 238.55 · конв 81.65% · 2317 покупців.

### ⚠️ Відкрите питання (потрібне рішення)
MV не фільтрує угоди по датах узагалі. Якщо сайт у новому циклі шле **стару** мітку — оплати мовчки лягають у старий проєкт. Живий приклад: **905 оплат (167 860 ₴) з 02-05.07** йшли як `IPHONE 17 PRO MAX` і досі зараховані червневому промо замість липневого `iphone_17_jul2026`. Загальні цифри від цього не страждають (подвійного рахунку нема), але розподіл між двома промо неправильний. Фікс = додати у MV фільтр по `date_start..date_end`, що змінить історичні цифри всіх проєктів — тому не роблю без відмашки.

---

## 14.08.2026 — Аудит «загублених полів»: поіменні SELECT vs реальна схема

### Що це за клас багів
Запит виду `.select('a,b,c')` або мапінг `map(x => ({ a: x.a, b: x.b }))` мовчки **викидає** колонки, яких у переліку нема. Колонку додали міграцією, читає її інший файл — а до нього вона не доїжджає. Помилки в консолі нема, дані просто «зникають». Два агенти пройшли фронт і бекенд, порівнюючи кожен поіменний перелік із живою схемою 11 ключових таблиць.

### Знайдене й виправлене (P0)
- 🛡 **`role-gate.js` — дірка доступу.** Fallback на `auth_id_aliases` існував **лише в коментарі**, коду не було. Байєр, залогінений alias-акаунтом, не розпізнавався (`me=null`) → gate тихо виходив і показував **усі** розділи замість тільки «Виконавець».
- 🔧 **`retention-tg-autopost` — пости йшли самим заголовком.** SELECT не тягнув `body`, а композер пише текст саме туди (`preview_text` для TG-каналу у формі прихований). Плюс не тягнув `dm_only`/`send_mode` — DM-розсилка могла вилетіти у **публічний канал**. Додано `.or()`-фільтри, які виключають `dm_broadcast`.
- 🆕 **`retention-tg-autopost`**: кнопки (`tg_buttons`) і відеозамітка (`video_note_creative_id`) тепер реально використовуються — раніше зберігались у БД і ігнорувались при відправці. Для медіагрупи кнопки йдуть окремим повідомленням (обмеження TG API).
- 🛡 **HARD RULE `compressed_at IS NOT NULL` відновлено у трьох відеосендерах** — `tg-post-send`, `retention-tg-autopost`, `.github/scripts/tg-autopost-worker.sh`. Правило було задокументоване 30.06, але зникло під час пізніших переписувань. `compressed_status` брехливий: фронт ставить `'ready'` одразу при аплоаді, ще до компресії. `'failed'`/`'n/a'` навмисно **не** дефферимо, інакше публікація зависає назавжди.
- 🔧 **Глобальний пошук по публікаціях був мертвий.** `app-fts-search.js` селектив колонку `text`, якої в `publications` нема (вона зветься `text_body`) → PostgREST валив весь запит → пошук мовчки повертав 0. Аліас `text:text_body`.
- 🔧 **`app-core.js` mappedPubs** не переносив `approved_by`, `next_action_*`, `platform_dates` — їх читають `app-multi-approver-fix.js` і `app-next-action.js`, тож будь-який повний refresh Store обнуляв блок «Наступна дія» й галочки погоджень до перезавантаження сторінки.
- 🔧 **`refreshCreatives`** не тягнув `poster_url`/`is_hdr`/`compressed_at` — картка після компресії лишалась зі старим прев'ю.
- 🔧 **`global-search.js`**: `team_tasks` без `.is('deleted_at', null)` → видалені задачі показувались нарівні з живими.
- 🔧 **Каса `saveMove()`** не знав про валютні книги: писав суму завжди в `amount_uah`, тож виплата дивідендів **$500 з USD-книги лягала як 500 ₴**. Тепер логіка як у `saveTx` (головне поле → `qty`); для переміщення в Готівку грн-еквівалент став обов'язковим.
- 🆕 **Ретеншн: селектор рубрики.** Панель фільтрів і колір border-left картки читали `rubric_id` з #547, але поставити його не було чим — фільтри завжди показували 0. Додано селектор + міграція `023_retention_rubric_id.sql`. Фронт самозахищений (`Store.hasRubricCol`): якщо колонки нема, поле не рендериться і не пишеться.

### Нова інфраструктура
- 🆕 **`.github/workflows/apply-migration.yml`** — застосування довільного `.sql` через Supabase Management API (`SUPABASE_ACCESS_TOKEN`). Досі в репо **не було жодного способу** накотити міграцію автоматично: файли лягали в `hq/db/migrations/` і чекали ручного запуску. `dry_run` показує SQL без виконання; fail-closed на будь-що крім 2xx.

---

## 14.08.2026 — Бібліотека креативів: превʼю для відео (запит Віри)

### Діагноз
- У Бібліотеці (медіа-стіл / SMM / Ретеншн) відео показувались порожніми плитками з ▶, фото — нормально.
- Причина подвійна: (1) у відео `thumbnail_url` містить URL **самого mp4**, тож `<img src=…>` не рендериться; правильне джерело — `poster_url` (JPEG-кадр, який генерує compress-воркер, #389). (2) Ключове: `poster_url` **взагалі не пробрасувався у Store** — мапінг креативів у `app-core.js` перелічує поля поіменно, і його там не було (як і `is_hdr` з 13.08).
- Готовий фікс пріоритету існував у `app-patches.js`, але цей файл не підключений (той самий мертвий файл зі списку аудиту, що й `app-dispatch-hooks.js`).

### Фікс
- ✅ `app-core.js`: у мапінг креативів додано `poster_url` + `is_hdr`.
- ✅ `app-views.js` (бібліотека + стрічка публікації) і `retention/app-retention.js`: `poster_url` тепер має пріоритет над `thumbnail_url` для відео.
- ✅ Прод-перевірка: у бібліотеці **180 відео-постерів**, усі перевірені рендеряться (`naturalWidth>0`).
- 🔁 50 старих відео (травень–червень, до появи генерації постерів) не мають `poster_url` — поставлено на перегенерацію хвилями по 12; нові завантаження постер отримують автоматично.

## 14.08.2026 — Dashboard: порожній «Виконавець» — регресія guard'а з аудиту

### Діагноз
- Розділ «Виконавець» (#terms) показував **1 значень · 0 лідів**, хоча в БД по `utm_term` за 14.08: vira 371, artem 270, vadym 108, dreamcar 85. RPC `dashboard_agg_deals_with_traffic_core` під service_role повертав усі 7 значень — тобто дані й агрегація живі.
- 🔴 **Корінь — моя регресія від аудиту 08-09.08.** У guard-хелпері `_dash_viewer_ctx()` користувач шукався ЛИШЕ по `users.auth_id`. Але робочий акаунт дашборду `dreamcarua@gmail.com` — це **alias** акаунта Вадима (`users.auth_id_aliases`), яким користується решта системи (напр. boot Каси). Через це `u_role = null`, а wrapper на `null` повертає порожньо.
- Уражені були всі розділи через цей RPC: **Виконавець, Джерела, Кампанії, Тип трафіка, Оголошення, Комбінації**. Огляд працював (інший RPC без guard) — тому виглядало як часткова поломка.

### Фікс
- ✅ `_dash_viewer_ctx()` тепер матчить `u.auth_id = jwt.sub` **АБО** `jwt.sub = any(u.auth_id_aliases)`. Одна зміна лікує всі wrapper'и (`dashboard_agg_deals_with_traffic`, `dashboard_agg_by_person`).
- ✅ Перевірено емуляцією JWT: alias-акаунт → `u_role=ceo` + 7 значень; `anon` → 0 рядків (buyer-захист не ослаблено). Прод: розділ показує vira/artem.
- 📖 Правило в памʼять: будь-яка перевірка identity має враховувати `auth_id_aliases`; після зміни guard тестувати під alias-JWT, а не через MCP (там service_role маскує баг).

## 14.08.2026 — Dashboard: 0 оплат при 923 лідах — новий розіграш не був змаплений

### Діагноз
- Скарга «не підтягуються дані»: Огляд показував 923 ліди / **0 оплат / 0 виручки / 0 джерел**. У БД при цьому було **651 оплата на 140 167 ₴** — тобто ETL живий, ламався мапінг.
- 🔴 Новий розіграш «Три iPhone 17» треба заводити у **два реєстри**, а не був у жодному коректно:
  1. `launches.deal_aliases` — було `NULL` (матчинг по `code`='3iphone' не збігався з `project`='3 IPHONE' через пробіл) → нулі у P&L.
  2. `dashboard_projects.deal_project_values` — **запису не існувало взагалі**. Саме ця таблиця живить селект «Активний розіграш» і фільтрацію KPI; у Вадима проєкт було видно в селекті лише з **localStorage** (fallback `ACTIVE_MODELS`), тож фільтр шукав неіснуючий проєкт.

### Фікс
- ✅ `launches.deal_aliases = ['3 IPHONE','IPHONE 17 PRO MAX']` (потрібно для P&L).
- ✅ Створено запис у `dashboard_projects` (code `3iphone`, 14.08–16.08, ті самі values, status active).
- ✅ Перевірено на проді з **увімкненим фільтром**: 923 ліди · 649 оплат · 139 869 ₴ · 6 джерел · 14 кампаній.
- 🔧 `detect-unmapped-projects`: алерт слався **один раз назавжди** — його легко пропустити (спрацював о 09:20, проєкт висів до обіду). Тепер нагадує **кожні 3 год, поки не змаплено** (upsert замість insert).
- 📖 Правило записано в памʼять; пункт «Перевірити deal_aliases» уже є в чек-листі старту проєкту (HQ → Чек-лист).

## 13.08.2026 — SMM: затримка автопосту 44 хв — root cause і фікс (запит Вадима)

### Що сталось
- Пост «тест 2 ТГ 22:00» вийшов о **22:44**. Ланцюг за фактами: відео залито 21:44 → публікація на 22:00 → **компресія завершилась 22:41** → автопост о 22:44 (перший тік після готовності медіа). Тобто система правильно чекала відео (щоб не вийшов пост без нього), але чекала занадто довго.
- 🔴 **Корінь: GitHub Actions cron `*/3` (кожні 3 хв) фактично виконується раз на ~60 хв.** Реальні запуски 13.08: 12:06 → 13:40 → 14:43 → 15:40 → 16:35 → 17:36 → 18:33 → 19:39 (UTC), усі `event=schedule`. Сам encode триває 3 хв — чекання розкладу давало решту.
- 🔴 Другий чинник: `hq/app-dispatch-hooks.js` (миттєвий тригер компресії при заливці, ≤2 с) **не був підключений в index.html** — мертвий файл із списку аудиту. Тому жодного `workflow_dispatch` для compress не було.

### Фікс (два незалежні шари)
- 🆕 **DB-тригер `trg_dispatch_compress_on_video`** — INSERT відео зі статусом `pending` одразу викликає `dispatch-workflow` через `pg_net` (дебаунс 90 с, помилка dispatch ніколи не блокує завантаження). Працює завжди, незалежно від браузера. Перевірено живим тестом: `event=workflow_dispatch` через 1 секунду після вставки.
- 🔧 Підключено `app-dispatch-hooks.js` як другий шар (realtime INSERT → dispatch, поки хтось у HQ).
- Очікуваний ефект: відео-пости виходять вчасно (компресія стартує одразу, а не за розкладом GH).

## 13.08.2026 — SMM: діагноз «падає якість відео в ТГ» + HDR-бейдж (запит Вадима)

### Діагноз (фактами, не здогадками)
- Скарга Саші стосувалась `IMG_0139.MOV` — і виявилось, що **наш пайплайн його не чіпав**: 35 591 922 → 35 591 922 байт (100%, pass-through). Лог воркера: `Color: transfer=arib-std-b67 primaries=bt2020 pix=yuv420p10le HDR=yes` → `HDR pass-through (no encode)`.
- **Псує сам Telegram:** Bot API не підтримує HLG/PQ 10-bit, тому TG робить власний downconvert у H.264 8-bit SDR — грубо. Звідси різниця проти галереї телефону.
- SDR-відео проблем не мають: заміряно ffprobe'ом — вихід 1080×1920, 30 fps, **15,8 Мбіт/с** (втричі вище за оригінал), візуально без втрат. Побічне: файли роздуваються (12 МБ → 47 МБ), бо таргет = бюджет 49,5 МБ, а не якість — у беклог (CRF-режим зі стелею).
- 🔴 Кольори у ffmpeg НЕ чіпали — tone mapping вже провалювався двічі (#256 Hable, #291 Mobius+eq).

### Профілактика
- 🆕 `creatives.is_hdr` — compress-worker пише прапорець із ffprobe-детекту (PATCH після `Color:`); 22 наявні HDR-креативи промарковано ретроспективно.
- 🆕 HQ: бейдж **«⚠ HDR»** на картці креативу (`app-views.js` → `renderCreativeStripItems`) + підказка в tooltip — видно ще до публікації.
- 📖 Команді надіслано інструкцію в SMM-чат: знімати в SDR (iPhone → Камера → Формати → вимкнути HDR-відео).

## 11.08.2026 — Finance: модуль імпорту виписок для Давида (запит Вадима)

### dashboard.dreamcar.ua/finance → вкладка «📥 Імпорт виписок»
- 🆕 Давид (COO, **без доступу в Касу**) може заливати виписки прямо з Фінансів. Формати: **Excel/CSV** (SheetJS) + **PDF** (pdf.js). Автодетект **ОТП** (за заголовками .xls) і **ПУМБ** (за x-координатами PDF); для інших банків — ручний маппер колонок.
- 🛡 Заливка через **рольову SECURITY DEFINER RPC `kasa_import_statement`** (тільки ceo/coo/cfo) — пише в `kasa_transactions` в обхід kasa RLS whitelist, але лише для дозволених ролей. Завжди `excl_pnl=true` (не впливає на P&L). Дедуп по № документа. + `kasa_fop_balances()` — read-only баланси ФОП для Давида.
- 🔧 ПУМБ PDF-парсер: pdf.js округлює y інакше, ніж Python-pdfplumber → переписано на date-anchoring (±4 по y); верифіковано в Node — баланс сходиться до копійки (49 562,90). Вбудована **звірка з «Вихідним сальдо» виписки** — розбіжність показується ⚠, щоб не залити хибне.
- ✅ Prod-smoke: модуль вантажиться, RPC балансів повертає всі ФОП, 0 console-помилок.

## 11.08.2026 — Каса: фікс — qty не читалась назад (запит Вадима)

### dashboard.dreamcar.ua/kasa
- 🔧 **USD/крипта сума зберігалась, але показувала 0.** Причина: `fetchAllTx()` перелічує колонки поіменно у `cols`, і там **не було нової колонки `qty`** — тому валютна сума писалась у БД правильно (qty=3000), але при читанні назад не підтягувалась → картка/рядок показували 0. Додав `qty` у `cols`. + Дотяг транзакцій розширено на `crypto`/`other` книги (було лише `cash`), щоб їхні операції завжди вантажились. Онови сторінку — старі валютні операції одразу покажуться коректно.

## 11.08.2026 — Каса: валютні книги — сума у валюті (запит Вадима)

### dashboard.dreamcar.ua/kasa
- 🔧 **Пріоритет валют перевернуто** (було: грн головне+обовʼязкове, валюта другорядна). Тепер для книг з `currency ≠ UAH` (USD/EUR/USDT/BTC): головне поле операції — **«Сума · [CUR] *»** (→ `qty`), а **«Еквівалент, грн»** — опційне (→ `amount_uah`, лише для загального зведення на головній). Картка книги, колонка «Поточний баланс» і рядок операції показують суму **у валюті** (3000 USD), грн — дрібним «(≈134 500 ₴)» лише як довідка.
- 🔧 Виправлено CHECK `kasa_accounts_kind_check` — дозволяв лише bank/cash/dividends, тож книги crypto/other БД відхиляла (insert падав). Тепер +crypto,+other. Створено книги-приклади «Готівка USD», «Крипта (USDT)».
- ✅ Prod-рендер верифіковано: картка «3000 USD», форма «Сума · USD *» + опційний грн-еквівалент, 0 console-помилок. Загальний підсумок зверху сумує грн-еквіваленти.

## 10.08.2026 — Каса: фікс редагування дивідендів/переміщень (запит Вадима)

### dashboard.dreamcar.ua/kasa
- 🔧 **Опис (і сума/дата) не зберігались при редагуванні дивіденду чи переміщення в готівку.** Причина: `saveMove()` при `editTx` робив `update({excl_pnl, div_to})` — тобто оновлював лише службові поля, а `description`/`amount_uah`/`occurred_at` не чіпав. Поле опису було редаговане, але зміни мовчки відкидались. Тепер update включає опис+суму+дату в обох гілках (дивіденди + переміщення в готівку). Верифіковано: модалка move-режиму, поле редаговане, 0 console-помилок.
- ⚠️ Відомий edge (не критично): при зміні суми move-у парна «дзеркальна» операція в Готівці не переоновлюється — окремий кейс, за потреби дороблю.

## 10.08.2026 — Каса: книги «Крипта» та «Інше» (запит Вадима)

### dashboard.dreamcar.ua/kasa
- 🆕 Нові типи книг у Рахунках: **Крипта** і **Інше (поза рахунками)** — доходи поза банківськими рахунками вносяться вручну як операції (як готівка/дивіденди).
- 🪙 **Крипта = кількість + оцінка в грн**: у крипто-книги є поле «Валюта/актив» (USDT/BTC/…); в операції — «Кількість» (крипта) + «Сума UAH» (гривнева оцінка, сумується в загальний баланс Каси). Картка показує і грн-баланс, і кількість активу; рядок операції — «+50 000 ₴ (1000 USDT)».
- 👁 Вкладка «💵 Готівка» → «💵 Готівка / Крипта» показує cash+crypto+other; кнопка «+ Операція (готівка/крипта/інше)». Тільки в Касі (позиція грошей), P&L не чіпає.
- 💵 **Готівкова валюта (USD/EUR)**: механізм «кількість + оцінка в грн» тепер прив'язаний до валюти книги (currency ≠ UAH), не до типу. Тобто готівковий долар = книга «Готівка» з валютою USD → вносиш суму в USD (qty) + гривневу оцінку; картка показує «1000 USD» + грн-баланс. Поле валюти доступне для всіх ручних книг (готівка/крипта/інше).
- 📖 Міграція `docs/kasa/db/022_kasa_crypto_other.sql`: колонка `qty` на `kasa_transactions` (currency вже було на kasa_accounts). RLS без змін — та сама `kasa_is_allowed()`.
- ✅ Prod: 0 console-помилок, рендер крипто/інше верифіковано (qty-баланс, toggle поля). Запис — під акаунтом Каси (whitelist email).
