...
// #556 P0 off-by-one fix: data-date МАЄ будуватись з ЛОКАЛЬНИХ компонент дати,
// а НЕ через toISOString() (той конвертує local-північ у UTC → у Kyiv GMT+3
// '2026-07-01 00:00+03' стає '2026-06-30T21:00Z' → slice(0,10)='2026-06-30').
// Кожна клітинка носила data-date на добу МЕНШЕ за свій day.getDate() → drop
// на 1 липня писав 30 червня. Той самий клас багу, що RPC_KYIV_TIMEZONE_GOTCHA.
function ymdLocal(d) {
...
function renderMonth(d, pubs) {
  ...
  for (let i = 0; i < 42; i++) {
    ...
    const more = merged.length > 6 ? `<span class="more" data-date="${ymdLocal(day)}">+${merged.length-6} ще</span>` : '';
    html += `<div class="cal-day ${isOther?'other-month':''} ${isToday?'today':''}" data-date="${ymdLocal(day)}">
...
function renderWeek(start, pubs) {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    html += `<div class="week-col" data-date="${ymdLocal(day)}">
