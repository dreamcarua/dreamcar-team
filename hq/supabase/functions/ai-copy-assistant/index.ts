...
// з урахуванням платформи + brand voice DreamCar/snEco/Abris Art/Barpi,
...
//
// Відповідь:
//   { ok, text, hashtags: string[], cta: string, model, tokens_in, tokens_out }
//
// Secrets:
//   ANTHROPIC_API_KEY — ключ Anthropic API
//   HQ_AI_SECRET (опційно) — секрет для frontend → backend (без user JWT)
// =====================================================================
...
const BRANDS: Record<string, { voice: string; rules: string; examples: string }> = {
  dreamcar: {
    voice:
      "DreamCar — це українська платформа, де люди отримують доступ до AI-сервісу й паралельно беруть участь у розіграшах автомобілів мрії. " +
      "За 6 років розіграно 16+ авто, аудиторія — 30-40 тис активних учасників. " +
      "Тон: натхненний, людяний, без надмірного хайпу. Говоримо про мрію, спільноту, реальність шансу.",
    rules:
      "ЗАБОРОНЕНО використовувати слова: «шанс», «квиток», «лотерея», «розіграш», «приз» — в юридичному значенні. " +
      "Замість них: «учасник», «отримати авто», «AI-токени», «спільнота DreamCar». " +
      "Завжди підкреслюй: купляєш токени AI, авто — приємний бонус для активних учасників. " +
  ...
  sneco: {
    ...
    rules:
      ...
      "Eкологічно — перший еко-снек в Україні. " +
      "Українська мова за замовчуванням; англійська для експортних постів.",
    examples:
  ...
  barpi: {
    ...
    rules:
      "100% натуральне м'ясо/риба, без консервантів. " +
      "Безпечно для будь-якого віку тварини. " +
...
function buildPrompt(input: {
  ...
  audience?: string; tone?: string; length?: string; examples?: string[];
}): string {
...
ТОН: ${tone}
ДОВЖИНА: ${len}
${input.audience ? `ЦІЛЬОВА АУДИТОРІЯ: ${input.audience}` : ""}
