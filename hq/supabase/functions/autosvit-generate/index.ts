// «Автосвіт» :: генератор v3.7 — відео як головний продукт; async-режим проти 150s idle-ліміту; max_tokens 16000
...
const CRAFT_VIDEO = `ВІДЕО — ГОЛОВНИЙ ПРОДУКТ. ВИРОБНИЧЕ ОБМЕЖЕННЯ (ЗАЛІЗНЕ): Вадим записує себе САМ — статичний селфі-кадр, тільки обличчя і голос (говоряча голова). Він НЕ актор: жодних локацій, ходьби, предметів у руках, «стоїть біля авто». ВСЯ динаміка робиться МОНТАЖЕМ іншою людиною, дистанційно.

Тому кожен кадр — це ДВА окремі завдання:
- frame = завдання МОНТАЖЕРУ: що на екрані в цей момент і ДЕ ЦЕ ВЗЯТИ. Конкретно: «б-рол: нічна траса зі стоку», «скріншот Euro NCAP з балом 95», «фото Nissan Rogue з оголошення», «графіка: 3896 великим кеглем вилітає», «зум-панч на говорячу голову», «спліт-скрін: два фото батарей». ЗАБОРОНЕНО писати сюди дії Вадима.
- how = що робить ВАДИМ у своєму статичному кадрі: емоція, погляд, мікрожест руками в межах селфі-кадру, темп, пауза, брова. Тільки те, що можна зробити сидячи перед камерою.

РИТМ МОНТАЖУ: говоряча голова відкриває відео, зʼявляється на поворотах і закриває фінал; решту часу — б-рол/графіка поверх голосу (50-70% хронометражу). Зум-панч або зміна крупності ГГ кожні 2 біти. Кожна цифра — графікою на екран.

ГАЧОК (0-2 сек) — подвійний: перша ФРАЗА з цифрою/протиріччям + найсильніший б-рол або графіка одразу. ЗАБОРОНЕНО починати з «Привіт», «Сьогодні поговоримо», «Ти знав, що…», назви рубрики.

ВІДКРИТА ПЕТЛЯ: обіцянка на початку (не влазить у голос — давай оверлеєм), розвʼязка в передостанньому кадрі.

МОВА (vo): дослівні розмовні фрази до 12 слів (~3 слова/сек — перевіряй, що начитується в хронометраж). Місточки: «Але є нюанс», «І ось тут цікаве», «Дивись». Звʼязки між кадрами — «але/тому», не «і потім».

ОВЕРЛЕЙ (txt): 2-4 слова великими, ключова цифра/теза кадру. Це НЕ субтитри.

ФІНАЛ: питання-вибір глядачеві. Без «підписуйся».

ПРИКЛАД РІВНЯ (кадр гачка): vo: «Оця балка щойно вбила міф про вічну Тойоту» · frame: «б-рол: іржаве кріплення підвіски крупно (фото з recall-звіту) → зум-панч на ГГ» · txt: «48 000 ІРЖАВІЮТЬ» · how: «різкий погляд у камеру, палець вгору на "щойно"». ОСЬ ТАКИЙ РІВЕНЬ КОЖНОГО КАДРУ.`;

const CRAFT_VIDEO_STORY = `ФОРМАТ: СТОРІ-РІЛЗ 60-90 СЕКУНД (12-16 кадрів). Це ІСТОРІЯ з напругою і розвʼязкою, а не набір фактів. Виробничі правила ті самі: Вадим — говоряча голова у статичному кадрі; frame = завдання монтажеру (б-рол/скріншоти/графіка + де взяти), how = емоція/мікрожест Вадима; динаміка — тільки монтажем.

СТРУКТУРА:
1) ХОЛОДНИЙ ГАЧОК (0-3с): фраза, що ламає шаблон + найсильніший б-рол; петля-обіцянка голосом або оверлеєм.
2) СЕТАП (3-12с): герой/предмет/ситуація — через б-рол і графіку, не через акторство.
3) РОЗВИТОК (12-35с): 2-3 біти з наростанням; кожен біт = нова деталь + НОВИЙ б-рол.
4) ПОВОРОТ (35-50с): момент «ого, а я думав інакше»; найсильніший факт; ГГ повертається в кадр.
5) ЦІНА ПИТАННЯ (50-65с): що це означає глядачеві особисто — гроші/безпека/звичка.
6) РОЗВʼЯЗКА ПЕТЛІ (65-80с): виконуємо обіцянку з гачка; мораль одним реченням.
7) ФІНАЛ (80-90с): питання-вибір, що ділить коменти на два табори; ГГ у кадрі.

Емоційна крива: іронія → цікавість → серйозність → тепло → виклик.`;
...
Поверни СТРОГО цілісний JSON без markdown:
{
 ...
 "script_video": [{"sec":"0-2","frame":"МОНТАЖ: б-рол/скріншот/графіка/прийом + де взяти (НЕ дії Вадима)","txt":"оверлей 2-4 слова","vo":"ДОСЛІВНА розмовна фраза до 12 слів","how":"Вадим у статичному кадрі: емоція/погляд/мікрожест/пауза"}] — ${story ? "12-16 кадрів на 60-90 сек за СТРУКТУРОЮ СТОРІ-РІЛЗА" : "6-9 кадрів на 30-45 сек за правилами ВІДЕО"},
