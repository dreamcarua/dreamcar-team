...
// 🔴 02.08.2026: \b у JS НЕ працює з кирилицею (\w = лише [A-Za-z0-9_]), тому «#баг»
// не ловився, а «#bug» ловився. Замість \b — явний негативний lookahead на літери/цифри.
const TECH_TAGS = /(^|\s)#(баг|буг|bug|фікс|фикс|fix|питання|вопрос|ідея|идея|idea|тех)(?![а-яґєіїёa-z0-9_])/i;
...
async function captureTechRequest(
  ...
  // працюємо лише у дозволених чатах (поки — тільки DreamCar SMM)
  if (!TECH_CHATS.has(String(msg.chat.id))) return false;
  ...
  // Відповідь на МОЄ повідомлення (уточнення або пропозиція рішення) → чіпляємо до звернення
  if (repliedToBot) {
    const rid = (msg.reply_to_message as any).message_id;
    const { data: parent } = await supabase
      .from("tech_requests")
      .select("id, status, from_name")
      .or(`ack_message_id.eq.${rid},proposal_msg_id.eq.${rid},message_id.eq.${rid}`)
      .maybeSingle();

    if (parent) {
      await supabase.from("tech_requests").insert({
        chat_id: String(msg.chat.id), message_id: msg.message_id, thread_msg_id: msg.message_id,
        from_user_id: from.id || null, from_name: fromName,
        kind: "answer", parent_id: parent.id, text: clean.slice(0, 4000), status: "done",
      });
      ...
      if (parent.status === "proposed") {
        ...
        if (yes) {
          await supabase.from("tech_requests")
            .update({ status: "confirmed", confirmed_by: fromName, confirmed_at: new Date().toISOString() })
            .eq("id", parent.id);
          await tgSend(msg.chat.id, `✅ Прийнято <b>#${parent.id}</b> — беру в роботу.`,
            { silent: true, reply_to_message_id: msg.message_id });
        } else if (no) {
          await supabase.from("tech_requests")
            .update({ status: "wontfix", resolution: "відхилено у чаті: " + clean.slice(0, 200), resolved_at: new Date().toISOString() })
            .eq("id", parent.id);
          await tgSend(msg.chat.id, `👌 Зрозумів, <b>#${parent.id}</b> не робимо.`,
            { silent: true, reply_to_message_id: msg.message_id });
        } else {
          await tgSend(msg.chat.id, `📝 Записав до <b>#${parent.id}</b>. Врахую й повернусь.`,
            { silent: true, reply_to_message_id: msg.message_id });
        }
      } else {
        // відповідь на уточнююче питання → повертаємо в роботу
        if (parent.status === "need_info") {
          await supabase.from("tech_requests").update({ status: "in_progress" }).eq("id", parent.id);
        }
        await tgSend(msg.chat.id, `📝 Записав до <b>#${parent.id}</b>.`,
          { silent: true, reply_to_message_id: msg.message_id });
      }
      return true;
    }
    // reply на повідомлення бота, не пов'язане зі зверненням — далі як нове (якщо є тег/згадка)
    if (!hasTag && !mentioned) return false;
  }
  ...
  const ack = await tgSend(
    msg.chat.id,
    `${label} прийнято <b>#${row?.id}</b> — у черзі.\nРозберусь і напишу сюди, що пропоную зробити.`,
    { silent: true, reply_to_message_id: msg.message_id },
  );
  if (ack?.message_id) {
