// ОДНОРАЗОВА: у Касі (ФАКТ) дивіденди = реальний вихід коштів → рахуємо у витрати.
// Виключаємо лише внутрішні перекази та переміщення в готівку (excl_pnl без div_to).
// Guard ?key=dckasa-pnlfix-K3
import postgres from "npm:postgres@3.4.4";
const GUARD = "dckasa-pnlfix-K3";
const SQL = `
...
language sql stable security invoker as $$
  ...
  where not t.is_internal and not (t.excl_pnl and t.div_to is null)
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const d = await sql`select * from public.kasa_cashflow('2025-06-01', now()::date, 'month')`;
    await sql.end();
    return new Response(JSON.stringify({ ok: true, cashflow: d }, null, 2), { headers: { "Content-Type": "application/json" } });
  } catch (e) {
