...
## Закрито 05.09.2026 (мерджі за словом Вадима «ок»)

- ✅ **PR #2** — `verify-publication-ig` знімає крон-джобу і після ручного питання. Разом із чисткою 61 сміттєвої джоби 04.09 клас помилки закрито.
- ✅ **PR #3** — воркер стиснення перейшов на capped CRF одним проходом. Креатив `ba4df100` повернуто в чергу і стиснуто з першої спроби: 27.3 MB, `ready`, 21:29 UTC.
- ✅ **PR #4** — `Auto Cache Bust` пушить із rebase-повтором. Впав під час мерджу двох PR підряд, тому версії `?v=` не бампались; після фіксу ручний запуск бампнув до `202609052131-95a7cb1a`.
