...
| Health (схема `health`, `health/`, edge `health-*`) | проєкт DayWeft: хаб `dreamcarua/dayweft-memory`, код `dreamcarua/nof1-app` (→ `dayweft-app`) | тут — лише за запитом з тієї сесії | легасі-джерело даних DayWeft; з `health-*` у цьому репо лежить лише `health-ingest`, решта — тільки в Supabase |
...
| Health (легасі DayWeft, зона іншої сесії) | — | pg_cron: `health-dash-cache` кожні 10 хв; брифи 06:07 і 20:00 з ретраями; watchdog кожні 3 год + 05:40 UTC; `health-media-sweeper` кожні 5 хв — усі через `net.http_post` з `x-cron-secret` | Health Auto Export + Shortcuts на телефоні → `health-ingest` | pg_cron; не чіпати без сесії DayWeft |
...
| Квота GitHub Actions | спільна на весь акаунт (~20 паралельних слотів) | інші проєкти Вадима (`abrisart-dashboards`, `sneco`, `nof1-app`) | не додавати частих кронів без підрахунку хвилин |
| Каталог `health/`, edge `health-*`, схема `health` | Вадим — через Cowork-сесію DayWeft (хаб `dayweft-memory`) | — | нічого без запиту з тієї сесії (24.08.2026: «не чіпай dreamcar — це задача іншого коворку» діє і в зворотний бік). Звіти health йдуть через edge `health-notify`, а не `cowork-notify` — персональні дані в публічний репо не потрапляють |

## Data boundaries

| Канал | Видимість | Може нести | Не має нести |
|---|---|---|---|
| Репозиторій (весь, включно з `docs/`) | публічний інтернет, `team.dreamcar.ua` | код, назви секретів, документація | значення секретів, chat id, хости, IP, PII учасників і команди, будь-які показники здоровʼя (інцидент 24.08.2026: 821 файл вичищено з історії) |
